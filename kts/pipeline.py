...
class Pipeline(ArithmeticMixin, PreprocessingMixin):
    ...
    def fit(self, **kwargs):
        ...
        if mb:
            ...
            pb.update(1)
        try:
            self.model.preprocess_fit(X, y,
                                      eval_set=[(self.featureslice(self.featureslice.idx_test).values,
                                                 self.featureslice.target.values[self.featureslice.idx_test])], **kwargs)
        except:
            if mb:
